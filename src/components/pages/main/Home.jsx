...
const Home = () => {
  ...
  useCloseModal(modalOpen, setModalOpen, 'dialog', 'button');
  useSetUserInfo();
  ...
  return (
    <section className="flex flex-col bg-black_3 h-[100vh] max-w-[1600px] m-auto">
      <header className="flex justify-between px-4 py-3 bg-lighterGray text-base font-normal text-whiteV1">
        <h3 className="font-bold">{chatId ? `+${chatId.split('@')[0]}` : ''}</h3>
      ...
      <footer className="relative z-20 w-full min-h-16 bg-lighterGray">
        <div className={`flex ${chatId ? 'items-end' : 'items-center justify-center'} z-20 relative px-4 pb-[5px] min-h-16`}>
          {chatId ? (
            <MessageForm />
          ) : (
      ...
      {modalOpen && <AddChatModal setModalIsOpen={setModalOpen} />}
      {loading && <Spinner />}
    </section>
  );
};
